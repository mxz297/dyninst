...
Parser::finalize(Function *f)
{
    ...
    for (auto bit = blocks.begin(); bit != blocks.end(); ++bit) {
         ...
         for (auto eit = b->targets().begin(); eit != b->targets().end(); ++eit) {
             ...
             // If the tail call target is a PLT stub, then keep it as a tail call
             if (trg_func->obj()->cs()->linkage().find(trg_func->addr()) != trg_func->obj()->cs()->linkage().end()) continue;
